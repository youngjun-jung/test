...
const executeProcedure = require('../procedures/repairexpensesaia.procedure');
const logger = require('../../logger'); 

  // 비즈니스 로직
exports.getRepairexpensesaiachk = async (req, res) => {
...
/*
  // 프로시저 호출
  const data1 = await executeProcedure.callRepairexpensesaiaproc(year);
  ...
  if(gubun != '0')
    ...
    else
    {
        query = `SELECT A.YEAR, '전체' MONTH, A.NAME, SUM(XA) XA, SUM(XB) XB, SUM(XC) XC, SUM(XD) XD, SUM(XE) XE, SUM(XF) XF, SUM(XG) XG
                ...
                FROM PLAN_REPAIR_EXPENSES_AIA A, PLAN_REPAIR_EXPENSES_CODE B
                WHERE A.YEAR = :year
                AND A.NAME = B.SNAME(+)
                AND A.YEAR = B.YEAR(+)

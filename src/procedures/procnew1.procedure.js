...
// 프로시저 호출 함수
exports.callProcnewproc = async (year, procid, zin_in, zin_lo, zin_out, zin_out_in, s_in, s_dong, s_on, scode, zinc_trans, ca_trans, comments, labor, chk) => {
    let connection;
    try {
        ...
        const result = await connection.execute(
            `BEGIN 
                SP_PROC_NEW1(:year, :procid, :zin_in, :zin_lo, :zin_out, :zin_out_in, :s_in, :s_dong, :s_on, :scode, :zinc_trans, :ca_trans, :comments, :labor, :chk, :returncode); 
            END;`,
            {
                year: year, // 매개변수
                procid: procid, // 매개변수

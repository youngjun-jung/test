...
exports.getSimulplannewtotalfinalchk = async (req, res) => {
  ...
  query = `SELECT YEAR, SCODE, XA, XB, XC
          ...
          , CASE WHEN FN_REF_VALUE('아연괴생산', A.YEAR, '00', :procid) + (SELECT VALUE FROM PLAN_TRANSFER_ZINC WHERE YEAR = A.YEAR AND SCODE = 'PTZ001' AND PROCID = :procid) - (SELECT M_VALUE FROM PLANNING_MONTHLY_INVENTORY_MANUAL WHERE YEAR = A.YEAR AND SCODE = 'PNMIM0101') > (SELECT SUM(VALUE) FROM PLANNING_PLUG_MANUAL WHERE YEAR = A.YEAR AND SCODE IN ('PPML0001', 'PPML0002', 'PPML0003', 'PPML0004') AND PROCID = 'jminzzang') 
                 THEN ((FN_REF_VALUE('아연괴생산', A.YEAR, '00', :procid)  + (SELECT VALUE FROM PLAN_TRANSFER_ZINC WHERE YEAR = A.YEAR AND SCODE = 'PTZ001' AND PROCID = :procid) - (SELECT M_VALUE FROM PLANNING_MONTHLY_INVENTORY_MANUAL WHERE YEAR = A.YEAR AND SCODE = 'PNMIM0101') - (SELECT SUM(VALUE) FROM PLANNING_PLUG_MANUAL WHERE YEAR = A.YEAR AND SCODE IN ('PPML0001', 'PPML0002', 'PPML0003', 'PPML0004') AND PROCID = 'jminzzang')) / (SELECT SUM(VALUE) FROM PLANNING_PLUG_MANUAL WHERE YEAR = A.YEAR AND SCODE IN ('PPML0001', 'PPML0002', 'PPML0003', 'PPML0004') AND PROCID = 'jminzzang')) * FN_REF_VALUE('캐소드생산', A.YEAR, '00', :procid)

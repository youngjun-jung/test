  ...
  if(gubun != '0')
  {
      query = `SELECT GUBUN, YEAR, '' MONTH, SNAME NAME, SUM(XA) XA, SUM(XB) XB, SUM(XC) XC, SUM(XD) XD, SUM(XE) XE, SUM(XF) XF, SUM(XG) XG, SUM(XH) XH, SUM(XI) XI, SUM(XJ) XJ     
                    ...
                    FROM (SELECT DISTINCT X.*, A.*
                        ...
                        LEFT JOIN (SELECT * FROM BACKUP_PLAN_DEPRECIATION WHERE BACKUP_ID = :backupid AND GUBUN = :gubun) A
                        ON X.SNAME = A.NAME
                        )
  ...
  else
  {
      query = `SELECT GUBUN, YEAR, '' MONTH, SNAME NAME, SUM(XA) XA, SUM(XB) XB, SUM(XC) XC, SUM(XD) XD, SUM(XE) XE, SUM(XF) XF, SUM(XG) XG, SUM(XH) XH, SUM(XI) XI, SUM(XJ) XJ     
                ...
                , A.XV*B.XV XV, A.XW*B.XW XW, A.XX*B.XX XX, A.XY*B.XY XY, A.XZ*B.XZ XZ, A.XAA*B.XAA XAA, X.IDX
                    FROM (SELECT SCODE, SNAME, IDX FROM BACKUP_PLAN_DEPRECIATION_CODE WHERE BACKUP_ID = :backupid) X
                    , (SELECT * FROM BACKUP_PLAN_DEPRECIATION WHERE GUBUN = '1') A
                    , (SELECT * FROM BACKUP_PLAN_DEPRECIATION WHERE GUBUN = '2') B

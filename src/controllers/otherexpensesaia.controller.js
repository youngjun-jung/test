...
const executeProcedure = require('../procedures/otherexpensesaia.procedure');
const logger = require('../../logger'); 

  // 비즈니스 로직
exports.getOtherexpensesaiachk = async (req, res) => {
  ...
  if(gubun != '0')
    ...
    else
    {
        query = `SELECT A.YEAR, '전체' MONTH, A.NAME, SUM(XA) XA, SUM(XB) XB, SUM(XC) XC, SUM(XD) XD, SUM(XE) XE, SUM(XF) XF, SUM(XG) XG
                ...
                FROM PLAN_OTHER_EXPENSES_AIA A, PLAN_OTHER_EXPENSES_CODE B
                WHERE A.YEAR = :year
                AND A.NAME = B.SNAME(+)
                AND A.YEAR = B.YEAR(+)
